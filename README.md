...
The Dometic RML10 controller provides a relay output for controlling the heating element for 12 V absorber fridge operation. Whereas the coil of the 12 V automotive relay is switched on the negative side using an open collector output.

The AMS1117 linear voltage regulator is connected between the positive relay terminal and ground. A 300 mA polyfuse and a diode have been added for overcurrent and reverse polarity protection.

The digital pin 12 is connected to the negative relay terminal via a polarity protection diode. The RML10 controller enables 12 V cooling by pulling down digital pin 12 via the negative relay terminal. When no cooling is required, the negative relay terminal stays afloat and pin 12 is set to high via its internal pullup resistor.

The TLP127 (LTV-252T) optocoupler is used to electrically isolate the RML10 controller from the BD35F compressor unit. It is being controlled by a PWM signal generated via AnalogWrite() over pin 11. The PWM output characteristics has been described in the next section.

The common (C) terminal of the BD35F controller represents the negative side and is connected to the TLP127 emitter. The temperature sensor (T) pin is connected to its collector.

Filtering capacitors C1 and C2 are required for the stable operation of the AMS1117 linear regulator. Bypass cpacitor C3 is added as an additional measure to cope with the electrical noise generated by the RML10's gas ignition system.
