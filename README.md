...
- The **TLP127 optocoupler (U1)** isolates the Raspberry Pi GPIO from the 12 V power supply. This allows the Pi to control the RML10’s D+ input.
  - **D+ enabled:** The RML10 uses ignition-controlled 12 V power (absorption cooling disabled if the towing vehicle is disconnected).
  - **D+ disabled:** The RML10 switches to propane refrigeration.

- **Diodes D1 and D2** form a simple OR circuit that merges the D+ signals from the Raspberry Pi and the towing vehicle. This ensures that the fridge receives a valid D+ signal if either source provides it.
...
The RV is equipped with a Raspberry Pi (RPi) running Victron Venus OS. The RPi collects data from multiple sources, including:

- Battery shunt (battery charge and current)
- Solar charge controller
- Truma Combi heater (indoor temperature)
- Fridge and outdoor BLE temperature sensors

Using this information—such as indoor and fridge temperatures, as well as battery charge level—the RPi can run an optimized fridge control algorithm to improve energy efficiency.

A Node-RED flow on the RPi sends commands to an Arduino based on the sensor data. The available remote control commands are:
...
| `c 22`    | Disable periodic defrost in EEPROM           |
| `c 23`    | Enable periodic defrost in EEPROM            |
| `c 30`    | Request system status information            |

Communication between the RPi and Arduino is handled wirelessly over UART using an HC-05 Bluetooth module. Alternatively, an FTDI-compatible USB-to-serial converter can be used for a wired connection.

The Node-RED flow implementing the fridge control algorithm is available on GitHub:
[https://github.com/microfarad-de/inetbox2mqtt](https://github.com/microfarad-de/inetbox2mqtt)
